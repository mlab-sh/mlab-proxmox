//! `profile` — list, inspect, select and delete saved clusters.
...
pub enum ProfileCmd {
    ...
    #[command(alias = "ls")]
    List,
    /// Show one profile, with the token secret masked
    Show {
...
fn list(cfg: &config::ConfigFile) -> Result<()> {
    if cfg.profiles.is_empty() {
        if render::is_json() {
            ...
            ui::warning("no cluster yet; run `mlab-proxmox login`");
        }
        return Ok(());
    }
    ...
    let rows: Vec<Value> = cfg
        ...
        .map(|(name, p)| {
            serde_json::json!({
                ...
                "endpoint": p.endpoint().unwrap_or_else(|_| p.host.clone()),
                "token": p.token_id,
    ...
    for r in &rows {
        ...
        println!(
            "  {marker} {name}  {}  {}{tls}",
            r["endpoint"].as_str().unwrap_or_default(),
            r["token"].as_str().unwrap_or_default().dimmed(),
        );
    }
    println!();
