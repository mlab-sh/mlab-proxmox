...
One workflow builds mlab-proxmox for macOS and Linux and puts everything on the
...
assets. It is
[`.github/workflows/release.yml`](https://github.com/mlab-sh/mlab-proxmox/blob/main/.github/workflows/release.yml),
...
`Formula/mlab-proxmox.rb`, with the four fresh checksums. It is generated, so it
should not be edited by hand — and it does not exist in the repository until
the first release creates it. Committing one before there is anything to
checksum would mean inventing four hashes, which is worse than an absent
formula: `brew install` would fail verification instead of failing to find a
tap.
